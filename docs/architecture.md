...
5. **GitHub Actions CI**：`push`/`pull_request` → `main`。`ubuntu-latest` 上 `cargo fmt --all -- --check` 与 `cargo test --locked`。工具链 1.91.0（`dtolnay/rust-toolchain@master`，预编译），依赖缓存 `Swatinem/rust-cache@v2`。`actions/checkout@v5`（Node 24）。test job 需要 secret `P2PCORE_TOKEN` 拉私有 git 依赖。不加 OS 矩阵、clippy、cargo-deny。
